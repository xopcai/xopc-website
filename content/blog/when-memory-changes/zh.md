...
> 文中的对话与时间均为说明机制而构造的示例。实现说明对应文末链接的代码快照；测试验证的是数据与上下文规则，不代表模型能在每段自然对话里准确识别用户意图。
...
这解决了一个容易漏掉的时间差：假设一条记忆十点过期，后台十一点才清理，十点半的回答也不该继续使用它。现有测试专门覆盖了维护任务运行之前的过期检查。
...
我们用几组小测试来检查这些规则。它们不需要一段看起来很聪明的演示回答，而是直接检查记录状态、有效期、替代关系和进入上下文的资格。

| 输入或变化 | 检查什么 |
| --- | --- |
| 重复表达同一偏好 | 复用记录，slot 数量不增加 |
| 明确纠正原偏好 | 新记录关联旧记录，旧有效期关闭 |
| 推测与明确偏好相反 | 推测进入冲突状态，明确偏好保留 |
| 两条记录有效期不重叠 | 按查询时间解析，不把历史变化当冲突 |
| 维护任务还没运行，记录已经过期 | 使用检查直接拒绝 |
| 自动激活的推测风险升高 | 仍须通过置信度、后果与敏感性检查 |
| 用户删除后再次自动提取 | 抑制自动重建，保留显式恢复路径 |

这些测试能说明规则按预期执行，不能说明“记忆问题已经解决”。从一句自然语言里提取对的属性、区分当前请求和长期偏好、找到准确的纠正目标，仍然可能出错。词法相关性也会漏掉表达不同但意思相近的内容；有限预算下，有用的信息可能没有被选中。

这也是为什么我们更愿意把记忆看成一份可以修订、有来源、有使用条件的记录。助理需要记住你，也需要允许你改变主意。对我们来说，一个很实际的检查标准是：当你说“之前那个不对，以后改成这样”，旧理解是否真的退出了下一次回答。

**在 xopc 里看看这些信息**
从产品探索了解用户理解与记忆相关能力，或查看下面的实现。要检查一次纠正，既要看记录是否更新，也要看后续回答实际使用了什么。
[探索 xopc 的能力 →](/zh/product-map)

## 实现与测试

本文核对于 2026 年 9 月 21 日。链接固定到同一代码快照，便于对照文章阅读；后续版本的行为可能调整。

- [事实位置、重复检测、纠正与冲突协调](https://github.com/xopcai/xopc/blob/a2a1fb40af4dc42fc35416ded195b573ab5b8977/src/user-model/repository.ts)

- [临时假设与使用资格](https://github.com/xopcai/xopc/blob/a2a1fb40af4dc42fc35416ded195b573ab5b8977/src/user-model/usage-policy.ts)

- [上下文筛选、排序与预算](https://github.com/xopcai/xopc/blob/a2a1fb40af4dc42fc35416ded195b573ab5b8977/src/agent/context/execution-context.ts)

- [纠正、冲突、历史与删除测试](https://github.com/xopcai/xopc/blob/a2a1fb40af4dc42fc35416ded195b573ab5b8977/src/user-model/__tests__/user-model.test.ts)
