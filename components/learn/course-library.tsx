...
export function CourseLibrary({ locale, initialCourse }: { locale: Locale; initialCourse: string }) {
  ...
  return <div className="landing-page product-atlas learn-page">
    <header className="pm-header"><Link className="pm-brand" href={`/${locale}`} aria-label="xopc"><XopcLogoMark/><b>xopc</b></Link><Link className="pm-home" href={`/${locale}/product-map`}><ArrowLeft size={16}/>{zh ? "产品地图" : "Product map"}</Link><div className="pm-header-tools"><a href={`/${other}/learn${selected ? `?course=${selected}` : ""}`} lang={other}>{zh ? "English" : "中文"}</a><Link className="pm-primary" href={`/${locale}#download`}>{zh ? "下载 xopc" : "Get xopc"}</Link></div></header>
    <main className="learn-main"><section className="learn-intro"><p className="pm-eyebrow">{zh ? "场景实战 · PC 端" : "PRACTICAL WORKFLOWS · DESKTOP"}</p><h1>{zh ? "从一件真实的事开始。" : "Start with something you need to do."}</h1><p>{zh ? "带着材料进入 xopc，核对过程，带走一份能继续使用的结果。" : "Bring your materials into xopc, review the work and leave with a useful result."}</p><div className="learn-tags"><span>{zh ? "6 个完整场景" : "6 complete workflows"}</span><span>{zh ? "中文配音与字幕" : "Chinese narration & captions"}</span><span>{zh ? "可下载跟做材料" : "Downloadable practice files"}</span></div></section>
    <section className="learn-grid" aria-label={zh ? "选择场景课程" : "Choose a course"}>{courses.map(c => {
    ...
    })}</section><section className="learn-guide"><h2>{zh ? "怎样跟着做" : "How to follow along"}</h2><ol><li>{zh ? "先完成模型接入，下载课程中的虚构练习材料。" : "Connect a model and download the fictional practice files."}</li><li>{zh ? "观看章节，暂停视频，在自己的 xopc 中执行同一步骤。" : "Watch a chapter, pause and try the steps in your own xopc."}</li><li>{zh ? "核对来源、日期与实际保存结果，再换成自己的授权资料。" : "Check sources, dates and saved results before using your own authorized materials."}</li></ol><Link href={`/${locale}/product-map?node=models`}>{zh ? "回到功能地图，补齐基础操作" : "Review the basics in the product map"}<ArrowRight size={16}/></Link><p>{zh ? "视频使用真实 PC 界面操作截图讲解。示例人物和业务材料为虚构，英文页面中的视频与详细教程仍为中文。" : "Videos use narrated screenshots from real desktop interactions. People and business examples are fictional. Videos and detailed guides are currently in Chinese."}</p></section></main>
